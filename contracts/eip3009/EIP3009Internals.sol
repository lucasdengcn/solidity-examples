...
abstract contract EIP3009Internals is ERC20, EIP712 {
    ...
    /**
     * @dev authorization deadline has expired.
     */
    error ERC3009AuthorizationExpired(uint256 deadline);

    /**
     * @dev authorization only valid after.
     */
    error ERC3009AuthorizationNotValidAfter(uint256 validAfter);

    /**
     * @dev Mismatched signature.
     */
    error ERC3009InvalidSigner(address signer, address owner);

    /**
     * @dev Mismatched sender on receive action.
     */
    error ERC3009SenderNotMatch(address sender, address owner);
    /**
     * @dev Mismatched signature.
     */
    error ERC3009AuthorizationUsed(address sender, bytes32 nonce);
    ...
    function _transferWithAuthorization(
        ...
        _verifySignature(data, from, v, r, s);
    ...
    function _receiveWithAuthorization(
        ...
        if (to != msg.sender) {
            revert ERC3009SenderNotMatch(msg.sender, to);
        }
        _requireValidAuthorization(from, nonce, validAfter, validBefore);
    ...
    function _cancelAuthorization(address authorizer, bytes32 nonce, uint8 v, bytes32 r, bytes32 s) internal {
        ...
        _verifySignature(data, authorizer, v, r, s);
    ...
    function _requireValidAuthorization(
        ...
        if (block.timestamp < validAfter) {
            revert ERC3009AuthorizationNotValidAfter(validAfter);
        }
