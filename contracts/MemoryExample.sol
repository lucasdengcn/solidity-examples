...
contract MemInternalFuncReturn {
    ...
    function internal_func_return_mem() private pure returns (bytes32[] memory) {
    ...
    function test_mem() public pure returns (uint256 len, bytes32 a0, bytes32 a1, bytes32 a2) {
...
contract Target {
    ...
    function return_uint256_arr(uint256 n) public pure returns (uint256[] memory) {
...
contract YulStaticCall {
    ...
    function test_abi_decode_uint256(address a, bytes calldata data) public view returns (uint256) {
        test_staticcall(a, data);
    }

    function test_abi_decode_bytes(address a, bytes calldata data) public view returns (bytes memory) {
        test_staticcall(a, data);
    }

    function test_abi_decode_uint256_arr(address a, bytes calldata data) public view returns (uint256[] memory) {
        test_staticcall(a, data);
    }
...
contract YulCall {
    function test_call(address a, bytes memory data) public payable returns (bytes memory out) {
        assembly {
            // 0x80
            let data_ptr := data
            // 0x60
